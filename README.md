...
- `VKUSVILL_BOT_TOKEN` — токен Telegram бота
...
Свободный ввод:
- `борщ` — разложит блюдо на ингредиенты и создаст корзину
...
├── resolver.py        # Резолвер блюд → продукты
...
1. Пользователь пишет `блюдо борщ` или `продукты молоко, хлеб`
2. Для блюда: `resolver.py` ищет рецепт в `recipes.yaml`
3. `vkusvill.py` ищет каждый товар через MCP API ВкусВилла
4. Для всех найденных товаров создаётся share-ссылка на корзину
5. Бот возвращает результаты и ссылку
